<template>
    <div
        class="col-xl-9 col-lg-8 personal-chat-container personal-fixed-chat--container room-container chat-container-992"
    >
...
export default {
    ...
    mounted() {
        ...
        Event.$on("added_message", message => {
            ...
            if(message.selfMessage == true){
            ...
            else if(message.type == 'admin' && this.type == 'admin'){
                if((message.receiver_id == Laravel.user.id)){
                    this.messages.push(message);
                }
            }
                ...
                if(message.receiver_id == Laravel.user.id && (message.room_id == this.roomId || this.roomId == undefined)){
                    this.messages.push(message);
                }
            }
            const height = $(".room-container .chat-container-main--info").height();
            $(".room-container .chat-container-main").animate({ scrollTop: height });
        });
    ...
    methods: {
        ...
        archive() {
            ...
            .then(response => {
                Event.$emit("archiveRoom", this.roomId);
            });
        },
        selectMessages(e){
