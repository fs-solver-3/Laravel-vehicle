    ...
    <section class="breadcrumbs breadcrumbs-public-pages">
      <div class="container">
        <ul class="breadcrumbs--list">
          ...
          <a class="breadcrumbs--item" href="{{ route('posts-all', app()->getLocale()) }}">@lang('front.index_page.posts')
              <svg class="icon icon-arrow-right3 ">
              <use xlink:href="{{ asset('static/img/svg/symbol/sprite.svg#arrow-right3') }}"></use>
              </svg>
          </a>
        ...
        <div class="blog-info--wrap">
            ...
            <div class="blog-info-article--date">
              ...
              <div class="blog-info-date">
                @php
                    ...
                    $date = date('F', strtotime ( $blog->created_at )); 
                    $date_month = str_replace($en_month, $ru_month, $date);
                    $date_day = str_replace($en_weekdays, $ru_weekdays, date('l', strtotime ( $blog->created_at)));
                @endphp  
                <span>{{ date('j', strtotime ( $blog->created_at )) }}  {{$date_month}}, {{ date('yy', strtotime ( $blog->created_at )) }}</span>
                </div>
            </div>
          ...
          <div class="col-xl-3 col-lg-3 col-md-12 col-sm-12 blog-info-sidebar--col pr-0">
            <div class="blog-info-sidebar--sticky">
              ...
              <a href="{{route('posts-all',  app()->getLocale())}}">
                <div class="gogocar-yellow-button">@lang('front.news.all_posts')</div>
              </a>
            </div>
          </div>
          @endif
