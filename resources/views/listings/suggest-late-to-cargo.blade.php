...
<div class="content">
    ...
    <section class="suggest-late">
        <div class="container">
            <form id="to_position_cargo" action="javascript:void(0)" enctype="multipart/form-data">
                <div class="suggest-late--wrap">
                    <h1 class="main-section--title text-center mb-5">@lang('front.suggest_late_to.where') <span>@lang('front.suggest_late_to.we_going_cargo') ?</span></h1>
                    <div class="col-xl-8 col-lg-8 suggest-late--inputs">
                        <div class="gogocar-input--wrapper">
                            ...
                            <input class="gogocar-input-v1 suggest-late--input--to map-input" id="address-input"
                                type="text" name="dc" placeholder="Санкт Петербург, Санкт-Петербург">
                        ...
                        <div class="trip-stops--places--buttons">
                            <a class="gogocar-gray-button m-0-auto-none mr-4" href="{{ route('suggestFromCargo', app()->getLocale()) }}">@lang('front.trip_stops_places.back')</a>
    ...
    <div class="modal-dialog modal-dialog-centered popup-modal--wrap" role="document">
        <div class="modal-content popup-content">
            <div class="popup-covid--wrap popup-icon-size">
                ...
                <div class="gogocar-yellow-button w-170" id="finish2"  data-dismiss="modal">Ок</div>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('#to_position_cargo').submit(function(e) {
            e.preventDefault();
            ...
            if(lat == 0 || lng == 0 || address == 0){
                ...
                var formData = new FormData($("#to_position_cargo")[0]);
                ...
                $.ajax({
                    ...
                    url: '{{ route('deployCargo', app()->getLocale()) }}',
                    data: formData,
                    cache:false,
                    contentType: false,
                    processData: false,
                    success: (data) => {
                        if (data.state == 'success') {
                            if (data.result == 'ok') {
                                window.location.href = '{{ route("stopsListings_cargo", app()->getLocale()) }}';
                            }
                            else if(data.result == 'draft'){
                                window.location.href = '{{ route("to_detail_cargo", app()->getLocale()) }}';
                            }
                        ...
                        // if (data == 'success') {
                        //     window.location.href = '{{ route("map_Deploy_Cargo", app()->getLocale()) }}';
                        // }
                        // else if(data.result == 'draft'){
