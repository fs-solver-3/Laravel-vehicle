...
<div class="content">
    ...
    <section class="suggest-late">
        <div class="container">
            <form method="GET" id="from_position_cargo" action="javascript:void(0)" enctype="multipart/form-data">
                <div class="suggest-late--wrap">
                    <h1 class="main-section--title text-center mb-5">@lang('front.suggest_late_from.where_from') <span>@lang('front.suggest_late_from.where_leaving_cargo') ?</span></h1>
                    <div class="col-xl-8 col-lg-8 suggest-late--inputs">
                        <div class="gogocar-input--wrapper">
                            ...
                            <input class="gogocar-input-v1 suggest-late--input--from map-input" id="address-input"
                                type="text" name="fc" placeholder="Например, Медресе Абулкосим, Ташкент">
...
<script>
    $(document).ready(function() {
        ...
        $('#from_position_cargo').submit(function(e) {
            e.preventDefault();
            ...
            if(lat == 0 || lng == 0 || address == 0){
                ...
                var formData = new FormData($("#from_position_cargo")[0]);
                ...
                $.ajax({
                    ...
                    url: '{{ route('suggestToCargo', app()->getLocale()) }}',
                    data: formData,
                    cache:false,
                    contentType: false,
                    processData: false,
                    success: (data) => {
                        if (data == 'success') {
                            window.location.href = '{{ route("suggest_cargo_to", app()->getLocale()) }}';
                        }
                        else if(data == 'failed'){
