    ...
    <section class="trip-stops--places">
        <div class="container">
            <div class="trip-stops--places--wrap">
                <h1 class="col-xl-9 col-lg-9 main-section--title--v2 text-center mb-5 m-0-auto">@lang('front.trip_stops_places.would_be_here'). <span>@lang('front.trip_stops_places.suits_you')?</span></h1>
                <ul class="trip-stops--places--list">
                    <div class="trip-stops--places--end mb-100px"><span>{{Session::get('from-position-passenger')}}</span></div>
                    @if(Session::get('stops-passenger-reserve')!= null)
                        @foreach(Session::get('stops-passenger-reserve') as $item)
                ...
                <div class="trip-stops--places--start"><span>{{Session::get('to-position-passenger')}}</span></div>
                </ul>
                <div class="trip-stops--places--buttons">
                    <a class="gogocar-gray-button m-0-auto-none mr-4" href="{{ route('stopsPassenger.show', app()->getLocale()) }}">@lang('front.trip_stops_places.back')</a>
                    <a class="gogocar-yellow-button m-0-auto-none" href="{{ route('whenTripPassenger', app()->getLocale()) }}">@lang('front.suggest_late_to.continue')</a>
                </div>
            </div>
        </div>
    </section>
    ...
    $(document).ready(function() {
        ...
        function gonext() {
            ...
            $.ajax({
                ...
                url: '{{ route("updateStops", app()->getLocale()) }}',
                data: {
                    active: active
                },
