    ...
    <section class="you-trip">
        <div class="container">
          <div class="you-trip--wrap">
            ...
            <p class="section-desc">@lang('front.profile_trips.here_you')!</p>
            ...
            <div class="col-xl-8 col-lg-8 you-trip--content m-0-auto">
                ...
                @if(count($trips) > 0)
                    ...
                    <div class="driver-trips">
                        ...
                        <div class="car-to-book--item--top">
                          <span class="car-to-book--item--date">
                            {{Helper::dateFormat($trip->starting_date)}}
                            {{date('H:i',strtotime($trip->starting_date))}}</span>
                          @if(!is_null($trip))
                              <div class="gogocar-trip--item__top">
              ...
              <div class="gogocar-trip--item companion">
                @if(count($booked_trips) > 0)
                    @foreach ($booked_trips as $trip)
                    @php
                      if($trip->listing->car_id!=null)
                      $trip_type1="passenger";
                      elseif($trip->listing->truck_id!=null)
                      $trip_type1="cargo";
                      else
                      $trip_type1=null;
                      @endphp
                    <a href="{{ route('tripplan', [app()->getLocale(), 't_id'=>$hashids->encode($trip->listing->id), 'type'=>$trip_type1]) }}">
                    <div class="driver-trips">
                        <div class="gogocar-trip--item__top">
                        <div class="car-to-book--item--top">
                          <span class="car-to-book--item--date">{{Helper::dateFormat($trip->starting_date)}} {{date('H:i',strtotime($trip->listing->starting_date))}}</span>
                            ...
                            <div class="gogocar-trip--item__top">
                                <div class="gogocar-trip--item-town__route">
                                    <div class="gogocar-trip--item__town">
                                      ...
                                      <span class="from_position">{{ $trip->listing->sourcecity->city }}</span>
                                      <div class="gogocar-trip--item__route" style="min-width: 100px; margin-top: 7px;">
                                          <div class="gogocar-trip--item__route--start"></div>
                                          <div class="gogocar-trip--item__route--end"></div>
                                      </div>
                                      <span class="to_position">{{ $trip->listing->destinationcity->city }}</span>
                                      <div class="gogocar-drivers--time__icon gdt-green">
                                        <svg class="icon icon-location2 ">
                                            <use xlink:href="{{ asset('static/img/svg/symbol/sprite.svg#location2') }}"></use>
                                        </svg>
                          ...
                          @endif
                            <div class="you-trip--companions">
                              <div class="car-to-book--item--profile">
                                  ...
                                  @if(!is_null($trip->listing->user) && !is_null($trip->listing->user->avatar))
                                    <div class="header-user-profile--img" style="background-image:url('{{ asset($trip->listing->user->avatar) }}');"></div>
                                  @else    
                                    <div class="cat-to-book--item__img" style="background-image:url('{{ asset('/static/img/content/drivers-avatar/driver1.png') }}');"></div>
                                  @endif
                                    <div class="you-trip--companion"><span>{{ $trip->listing->user->name}}</span>
                                  </div>
                              </div>
                            </div>
                        </div>
                        <div class="car-to-book--item--bottom">
                            <div class="car-to-book--bottom-info">
                              <div class="car-to-book--place--price">{{ $trip->listing->seats()}} @lang('front.profile_trips.place'): <span class="money" data-currency-rub="{{Helper::convertCurrency($trip->listing->price_per_seat, 'UZS', 'RUB')}}" data-currency-uzs="{{$trip->listing->price_per_seat}}">
                                {{ $trip->listing->price_per_seat }} UZS
                                </span></div>
                              <div class="car-to-book--during">@lang('front.profile_trips.payment'): <span>@lang('front.profile_trips.on_the_way')</span>
                              </div>
                            </div>
                            <div class="gogocar-ready-plan--car">
                              <div class="gogocar-gray-icons">
                              ...
                              <div class="gogocar-ready-plan--text">
                                  @if(!is_null($trip->listing->car))
                                  {{$trip->listing->car->model}}
                                  @endif
                              </div>
                            </div>
                        ...
                        <div class="car-to-book--services">
                            ...
                            <div class="car-to-book--services--left--content"><span class="car-to-book--place--price">@lang('front.profile_trips.service_charge')</span><span class="car-to-book--during">@lang('front.profile_trips.no_ride_fee')</span></div>
                            </div>
                            <div class="car-to-book--services--right">
                            <div class="car-to-book--services--left--content">
                              <span class="car-to-book--during--right money" data-currency-rub="{{Helper::convertCurrency($trip->listing->price_per_seat, 'UZS', 'RUB')}}" data-currency-uzs="{{$trip->listing->price_per_seat}}">
                              {{ $trip->listing->price_per_seat }} UZS
                              </span>
                            </div>
                            </div>
            ...
            <a href="{{ route('trips.old', app()->getLocale()) }}">
              <div class="gogocar-gray-button">@lang('front.profile_trips.past_trip')</div>
            </a>
          </div>
        </div>
    </section>
