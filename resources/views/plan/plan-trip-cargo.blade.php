    ...
    <section class="ready-plan-trip">
        <div class="container">
            <div class="ready-plan-trip--wrap">
                <h1 class="main-section--title--v2 text-center mb-5">@lang('front.plan.trip_is_planned_for') <span>
                    @php
                        $date = date('l\, j F', strtotime ( $detail->starting_date )); 
                    @endphp   </span></h1>
                                        ...
                                        @foreach($stops as $item)
                                            ...
                                            @else
                                                <div class="gogocar-ready-plan-trip--info" style="">
                                                    <div class="gogocar-plan-trip-driver-position--town" style="padding: 0px;"><span>{{ $item->location->city }}</span></div>
                                                    ...
                                                    <div class="gogocar-plan-trip-driver-position--time" style="padding: 0px;">
                                                        <span>@lang('front.plan.not_expected')</span></div>
                                                </div>
                                            @endif
                                        @endforeach
                                    @endif
                        ...
                        <div class="gogocar-trip--item__bottom">
                            <div class="gogocar-ready-plan--bottom">
                                <div class="gogocar-ready-plan--km__place">
                                    ...
                                    <div class="gogocar-ready-plan--place">{{$detail->capacity}} @lang('front.plan.tons_of_free')</div>
                                </div>
                                ...
                                <div class="gogocar-ready-plan--covid">
                                    <div class="gogocar-gray-icons">
                                        <svg class="icon icon-fan ">
                                            <use xlink:href="{{ asset('static/img/svg/symbol/sprite.svg#fan') }}"></use>
                                        </svg>
                                    </div>
                                    <div class="gogocar-ready-plan--text">{{$detail->covid19_title}}</div>
                                </div>
                            </div>
        ...
        <div class="container">
            <div class="chat-wrap">
                ...
                <div class="chat-section" id="app">
                     <room-com :user="{{ $detail->carowner}}" :listing="{{ $detail->id }}" type="passenger">
                     </room-com>
                    <div class="col-xl-3 col-lg-4 chat-content--info">
                        <div class="chat-content--info__driver--profile gogocar-box">
                            ...
                            {{-- @if(count($reviews) > 0 )
                            <a class="gogocar-yellow-button chat-gogocar-yellow-button-desctop" href="/">@lang('front.plan.more_information')
                            </a>
                            @endif --}}
                            <div class="chat-content-buttons__driver--992">
                                {{-- <a class="gogocar-gray-button" href="/">@lang('front.plan.more_information')</a> --}}
                        ...
                        <div class="chat-content--info__driver--review gogocar-box chat-content--info__driver--mobile">
                            @if(count($reviews) > 0 )
                            <div class="chat-content--info__driver--review--header"><span class="chat-content--info__driver--title">@lang('front.plan.reviews')</span>
                                <div class="gogocar-trips-category--rating green-rating">
                                    <svg class="icon icon-green-star ">
                                        <use xlink:href="{{ asset('static/img/svg/symbol/sprite.svg#green-star') }}"></use>
                                    </svg><span>{{ round($average_score, 1)}}</span>
                                            ...
                                            <div class="chat-content--info__driver--review--content">
                                                <div class="chat-content--review__trip--profile">
                                                    <div class="chat-content--review__trip--info">
                                                        <div class="chat-content--review__trip--info__img" 
                                                        @if(is_null($item->writer->avatar))
                                                        ...
                                                        <div class="chat-content--review__trip--name__trip">
                                                            <span class="chat-content--review__trip--name">{{ $item->writer->name}}</span>
                                                            {{-- <span class="chat-content--review__trip--trip">{{ $item->booking->listing->location() }} → {{ $item->booking->listing->destination() }}</span> --}}
                                                        </div>
                                                    </div>
                                                ...
                                                <p class="chat-content--review__trip--text">{{ str_limit($item->comment, 60) }}</p>
                                                <span class="chat-content--review__trip--date">{{ date('d.m.yy', strtotime($item->created_at)) }}</span>
                                            </div>
                                            @endif
                                        @endforeach
                                    @else
                ...
                <div class="col-xl-9 col-lg-9 pl-0 plan-trip-btn">
                    {{-- <a class="gogocar-yellow-button" href="/">@lang('front.plan.continue')</a> --}}
                    <a class="gogocar-yellow-button" href="{{ route('checkbookingcargo', [app()->getLocale(), 'trip_id'=>$detail->id]) }}">@lang('front.plan.continue')</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <div class="modal-dialog modal-dialog-centered popup-modal--wrap modal-970" role="document">
        <div class="modal-content popup-content">
            ...
            <div class="popup-covid--wrap popup-icon-size">
                ...
                <p class="main-section--title font-size-20 text-center">@lang('front.modal.which') <span>@lang('front.modal.at_you')</span> @lang('front.modal.reason') <span>@lang('front.modal.for_a_complaint') ?</span></p>
