$(document).ready(function() {
    ...
    $('#directions_id').on('change', function() {
        var query = $(this).children("option:selected").data('direction-id');
        fetch_rerrals(query);
    });

    function fetch_rerrals(query) {
        $.ajax({
            url: "/fetch_courses?query=" + query,
    ...
    $('.datatable').each(function() {
        ...
        $("#search_title").on('change', function() {
            ...
            //     dom: 'lrtip'
            // });
            var options = $('#search_title').select2('data');
            // console.log(options);
            // var options = [];
            // options = $(this).val();
            ...
            dataTable.fnFilter(types, 2, true, false, false, false);
        });
        $('#directions_id').on('change', function() {
        ...
        $("#search_comment").keyup(function() {
            dataTable.fnFilter(this.value, 4);
        });
        $("#search_rate").keyup(function() {
            dataTable.fnFilter(this.value, 4);
        });
        ...
        $('#teachers').on('change', function() {
            ...
            console.log(types);
            //filter in column 0, with an regex, no smart filtering, no inputbox,not case sensitive
            dataTable.fnFilter(types, 3, true, false, false, false);
        });
        ...
        $('#publish').change(function() {
            if (this.checked) {
                dataTable.fnFilter('^1\$', 5, true, false, false, false);
            } else {
                dataTable.fnFilter('^0\$', 5, true, false, false, false);
            }
        });
